...
def run(
    ...
    dry_run: bool = typer.Option(
        ...
        "--dry-run",
        help=(
            "Run the full pipeline but do not open a PR; print the diff and "
            "sandbox test output instead."
        ),
    ),
) -> None:
    ...
    log_event(f"[run] {owner}/{name}#{issue} run {run_marker}{mode_suffix}")
    ...
    # Both outcomes want the audit trail, and a stalled run wants it most. In
    # dry-run, the whole point is seeing what would have shipped, so force the
    # verbose expansion regardless of the flag.
...
def dispatch(
    ...
    dry_run: bool = typer.Option(
        ...
        "--dry-run",
        help=(
            "Forward --dry-run to the container so the pipeline runs but no PR "
            "is opened."
        ),
    ),
) -> None:
